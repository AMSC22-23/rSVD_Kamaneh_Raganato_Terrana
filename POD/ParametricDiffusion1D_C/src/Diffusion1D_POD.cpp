...
DiffusionPOD::setup_reduced()
{
  ...
  // Initialize the linear system.
  {
    ...
    reduced_system_lhs.reinit(sparsity_pattern_r);

    pcout << "  Initializing the system right-hand side" << std::endl;
    reduced_system_rhs.reinit(dof_handler.n_dofs());
    pcout << "  Initializing the solution vector" << std::endl;
    reduced_solution.reinit(dof_handler.n_dofs());
  }
}

void
DiffusionPOD::assemble_matrices()
{
  pcout << "===============================================" << std::endl;
...
void
DiffusionPOD::assemble_rhs(const double &time)
{
  const unsigned int dofs_per_cell = fe->dofs_per_cell;
  const unsigned int n_q           = quadrature->size();
...
void
DiffusionPOD::assemble_reduced_rhs(const double &time)
{
  ...
  // Add the term that comes from the old solution.
  reduced_rhs_matrix.vmult_add(reduced_system_rhs, reduced_solution); // QUI TI SERVIREBBE NUOVA SOLUTION
  ...
  // Boundary conditions.
  {
    ...
    MatrixTools::apply_boundary_values(
      boundary_values, reduced_system_lhs, reduced_solution, reduced_system_rhs, false);
  }
}

void
DiffusionPOD::convert_modes(FullMatrix<double> &transformation_matrix)
...
void
DiffusionPOD::project_u0(FullMatrix<double> &transformation_matrix)
// AdvDiffPOD::project_u0(FullMatrix<double> &transformation_matrix)
{
  ...
  pcout << "Check solution owned copy" << std::endl;
  pcout << solution_owned(17) << std::endl;
  pcout << solution_owned_copy(17) << std::endl;
  pcout << solution_owned(20) << std::endl;
  pcout << solution_owned_copy(20) << std::endl;
  pcout << solution_owned(50) << std::endl;
  pcout << solution_owned_copy(50) << std::endl;
  pcout << solution_owned(70) << std::endl;
  pcout << solution_owned_copy(70) << std::endl;


  reduced_solution = 0.0;
  transformation_matrix.Tvmult(reduced_solution, solution_owned_copy); // qui potrebbe far casino
  pcout << "Blocco?" << std::endl;
  // reduced_solution_owned.compress(VectorOperation::add); // tolto compress
...
void
DiffusionPOD::project_lhs(FullMatrix<double> &transformation_matrix)
...
////////////////
  ...
  pcout << "BLOCCO QUI 2" << std::endl;
  assert(aux.n() == transformation_matrix.m());
  aux.mmult(dst, transformation_matrix); // prova senza dst
  // Maybe dst is necessary since mmult can't be applied to mixed sparse and full matrices
  pcout << "Check res size" << dst.m() << "*" << dst.n() << std::endl;
  // aux.mmult(reduced_system_lhs, transformation_matrix);
  pcout << "BLOCCO QUI 3" << std::endl;
  ...
  //     reduced_system_lhs.set(i, j, dst(i, j));
  reduced_system_lhs.copy_from(dst); // eventualmente togliere prova questo copy_from
  ...
  // reduced_system_lhs.compress(VectorOperation::add); // tolto ora
  pcout << "BLOCCO QUI 4" << std::endl;
...
void
DiffusionPOD::project_rhs(FullMatrix<double> &transformation_matrix)
// AdvDiffPOD::project_rhs(FullMatrix<double> &transformation_matrix)
{
  ...
  transformation_matrix.Tvmult(reduced_system_rhs, system_rhs_copy); // anche qui prova senza dst
  // reduced_system_rhs.compress(VectorOperation::add);
...
void
DiffusionPOD::project_rhs_matrix(FullMatrix<double> &transformation_matrix)
// AdvDiffPOD::project_rhs_matrix(FullMatrix<double> &transformation_matrix)
{
  ...
  // controlla il copy from
  pcout << "  Check rhs_matrix_copy: " << std::endl;
  pcout << rhs_matrix(0, 0) << std::endl;
  pcout << rhs_matrix_copy(0, 0) << std::endl;
  pcout << rhs_matrix(1, 0) << std::endl;
  pcout << rhs_matrix_copy(1, 0) << std::endl;
  pcout << rhs_matrix(100, 70) << std::endl;
  pcout << rhs_matrix_copy(100, 70) << std::endl;
  pcout << rhs_matrix(120, 70) << std::endl;
  pcout << rhs_matrix_copy(120, 70) << std::endl;

  transformation_matrix.Tmmult(aux, rhs_matrix_copy);
  aux.mmult(dst, transformation_matrix); // anche qui senza dst

  // for (unsigned int i = 0; i < transformation_matrix.n(); ++i)
  //   for (unsigned int j = 0; j < transformation_matrix.n(); ++j)
  //     reduced_rhs_matrix.set(i, j, dst(i, j));
  reduced_rhs_matrix.copy_from(dst);
...
void
DiffusionPOD::solve_time_step_reduced()
{
  ...
  solver.solve(reduced_system_lhs, reduced_solution, reduced_system_rhs, preconditioner);
  pcout << "  " << solver_control.last_step() << " CG iterations" << std::endl;

  // reduced_solution = solution_owned;
...
void
DiffusionPOD::output(const unsigned int &time_step) const
{
