...
void
Poisson2D::set_parameters(std::vector<double> &prm_diffusion_coefficient)
{
  diffusion_coefficient = DiffusionCoefficient(prm_diffusion_coefficient);
}
...
Poisson2D::setup()
{
  ...
  // Initialize the linear system.
  {
    ...
    std::cout << "-----------------------------------------------" << std::endl;
    std::cout << "  Initializing the diffusion parameter" << std::endl;

    // Initialize the diffusion parameter.
    ...
    {
        double p = min + i*(max-min)/(num-1);
        prm_diffusion_coefficient.push_back(p);
    }

    set_parameters(prm_diffusion_coefficient);
}
      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          ...
          // Vector<double> beta(dim);
          // transport_coefficient.vector_value(fe_values.quadrature_point(q), beta);

          // Convert the transport term to a tensor (so that we can use it with
          // scalar_product).
          // Tensor<1, dim> beta_tensor;
          // for (unsigned int i = 0; i < dim; ++i)
          //   beta_tensor[i] = beta[i];
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  std::cout << "CHECK: " << diffusion_coefficient.value(
                                         fe_values.quadrature_point(q)) << std::endl;

                  // Transport term.
                  // cell_matrix(i, j) += scalar_product(beta_tensor,
