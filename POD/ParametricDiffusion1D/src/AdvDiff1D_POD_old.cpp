...
AdvDiffPOD::project_lhs(TrilinosWrappers::SparseMatrix &transformation_matrix)
{
  ...
  // FullMatrix<double> cell_lhs(dofs_per_cell_s, dofs_per_cell_s);
  ...
  //                 aux.mmult(cell_lhs, transformation_matrix);
  //                 // cell_lhs(i, j) = lhs_matrix(i, j);    
  //               }
  //           }
  //       }
  ...
  //     reduced_system_lhs.add(dof_indices_s, cell_lhs);
  //   }

  // reduced_system_lhs.compress(VectorOperation::add);


  // unsigned int local_start_row = (mpi_rank * transformation_matrix.size()) / mpi_size; // cambia nomi
  // unsigned int local_end_row = ((mpi_rank + 1) * transformation_matrix.size()) / mpi_size;

  // TrilinosWrappers::SparseMatrix aux;
  // reduced_system_lhs = 0.0;
  // for (unsigned int i = local_start_row; i < local_end_row; ++i) {
  //   for (unsigned int j = 0; j < transformation_matrix[0].size(); ++j) {
