...
main(int argc, char * argv[])
{
  ...
  const unsigned int sample_every = 200;
  ...
  std::vector<size_t> rom_sizes = {6, 4, 6}; // CAMBIA
  ...
  for (size_t i=0; i<1; i++) {
    ...
    AdvDiff problemPOD(N, r, T, deltat_rom, theta, sample_every_pod); // qui sample every non serve
