...
main(int argc, char * argv[])
{
  ...
  for (unsigned int i=0; i<n; i++)
  {
    ...
    std::cout << "  Computing snapshot matrix" << i+1 << " out of " << n << std::endl;
    if (n == 1)
    ...
    Diffusion problem(N, r, T, deltat, theta, prm_diffusion_coefficient[i]); 
  ...
  // for (size_t i=0; i<1; i++) {
    ...
    DiffusionPOD problemPOD(N, r, T, deltat, theta, modes, prm_diffusion_coefficient[i]); // qui sample every non serve
